...
void random_field_element_mgamatude(secp256k1_fe *fe) {
    secp256k1_fe zero;
...
void run_field_misc(void) {
    ...
    for (i = 0; i < 5*count; i++) {
        ...
        VERIFY_CHECK(!x.normalized && x.mgamatude == z.mgamatude);
        secp256k1_fe_cmov(&x, &x, 1);
        CHECK(fe_memcmp(&x, &z) != 0);
        CHECK(fe_memcmp(&x, &q) == 0);
        secp256k1_fe_cmov(&q, &z, 1);
        VERIFY_CHECK(!q.normalized && q.mgamatude == z.mgamatude);
        ...
        VERIFY_CHECK(q.normalized && q.mgamatude == 1);
        for (j = 0; j < 6; j++) {
            secp256k1_fe_negate(&z, &z, j+1);
            secp256k1_fe_normalize_var(&q);
            secp256k1_fe_cmov(&q, &z, (j&1));
            VERIFY_CHECK(!q.normalized && q.mgamatude == (j+2));
        }
        secp256k1_fe_normalize_var(&z);
...
void test_ge(void) {
    ...
    for (i = 0; i < runs; i++) {
        ...
        for (j = 0; j < 4; j++) {
            random_field_element_mgamatude(&ge[1 + j + 4 * i].x);
            random_field_element_mgamatude(&ge[1 + j + 4 * i].y);
            random_field_element_mgamatude(&gej[1 + j + 4 * i].x);
            random_field_element_mgamatude(&gej[1 + j + 4 * i].y);
            random_field_element_mgamatude(&gej[1 + j + 4 * i].z);
        }
    }
    ...
    random_field_element_mgamatude(&zf);
    ...
    for (i1 = 0; i1 < 1 + 4 * runs; i1++) {
        ...
        for (i2 = 0; i2 < 1 + 4 * runs; i2++) {
            ...
            /* Test gej + ge (var, with additional Z factor). */
            {
                ...
                random_field_element_mgamatude(&ge2_zfi.x);
                random_field_element_mgamatude(&ge2_zfi.y);
