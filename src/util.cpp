...
const char * const BITCOIN_CONF_FILENAME = "gama.conf";
const char * const BITCOIN_PID_FILENAME = "gamad.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "gama" is a composite category enabling all Gama-related debug output
            if(ptrCategory->count(string("gama"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GamaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GamaCore
    // Mac: ~/Library/Application Support/GamaCore
    // Unix: ~/.gamacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GamaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GamaCore";
#else
    // Unix
    return pathRet / ".gamacore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty gama.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override gama.conf
