...
#if defined(NDEBUG)
# error "Gama Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    int nHalving = consensusParams.nSubsidyHalvingInterval;
    int nHyper1= 7200;
    int nHyper2= 720;
    ...
    if (nPrevHeight == 0)
    	nSubsidy = 35000 * COIN;
    else if ((nPrevHeight+1) < nRampPeriod)
    	nSubsidy = static_cast<CAmount>(nSubsidy/nRampPeriod*nPrevHeight);
    else if ((nPrevHeight+1) % nHyper1 == 0)
      nSubsidy = 1000 * COIN;
    else if ((nPrevHeight+1) % nHyper2 == 0)
      nSubsidy = 100 * COIN;

    // yearly decline of production by 10% per year, 5.5M by year 2050
    for (int i = nHalving; i <= nPrevHeight; i += nHalving) {
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // GAMA : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(GAMA): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(GAMA): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // GAMA : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(GAMA): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(GAMA): spork is off, skipping transaction locking checks\n");
    }
